...
export default function RestaurantMap({ setSelectedRestaurant }: Props) {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative h-[600px] bg-yellow-50 rounded-md border border-yellow-200"
      ...
      {hoveredRestaurant && (
        ...
        >
          ...
          <p className="text-sm text-gray-600">
            Спрос:{' '}
            <span className="font-medium">{hoveredRestaurant.demand}</span>
          </p>
          <p className="text-sm text-gray-600">
            Продажи:{' '}
            <span className="font-medium">
              ₸{hoveredRestaurant.sales.toLocaleString()}
            </span>
          </p>
          <p className="text-sm text-gray-600">
            Посетители:{' '}
            <span className="font-medium">{hoveredRestaurant.visitors}</span>
          </p>
        </motion.div>
      )}
